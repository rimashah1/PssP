...
db = SQLAlchemy() # create new database connection using sqlalchemy
app = Flask(__name__)

app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://' + mysql_username + ':' + mysql_password + '@' + mysql_host + ':3306/patient_portal'
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False # keep track of db changes if True
app.secret_key = '343fdksjf34#$#dfjkhdf0SDJH0df9fd98343fdfu34rf' # sqlachemy connection to mysql requires a secret key
...
@app.route('/')
def index(): # define a function that specifies what happens when user goes to that url
...
def get_gui_patients():
    returned_Patients = Patients.query.all() # query all info/columns from Patients Class 
    return render_template("patient_all.html", patients = returned_Patients) # render html page and specify variable with data
